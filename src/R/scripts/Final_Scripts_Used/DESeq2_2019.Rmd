...
# ctsGC <- as.matrix(count_data$counts)
# sampleInfo <- read.csv("/Users/hollymcqueary/Dropbox/McQueary/Dosage-Compensation/Paper_Drafts/sample_data_rsubread.csv",header=TRUE)
...
```{r reorderData}
# colnames(ctsGC) <- sub("X.Users.hollymcqueary.Dropbox.McQueary.Dosage.Compensation.Indiv_Genes.Rsubread.bams.", "", colnames(ctsGC))
# 
# colnames(ctsGC) <- sub(".bam", "", colnames(ctsGC))
# colnames(ctsGC)
# all(rownames(sampleInfo) %in% colnames(ctsGC))
# all(rownames(sampleInfo) == colnames(ctsGC))
# rownames(sampleInfo) <- sampleInfo$sample
# ctsGC <- ctsGC[, rownames(sampleInfo)]
# sampleInfo <- sampleInfo[colnames(ctsGC),]
# all(rownames(sampleInfo) == colnames(ctsGC))
# rownames(sampleInfo)
...
```{r matrixInput}
#library("DESeq2")
# dds <- DESeqDataSetFromMatrix(countData = ctsGC,
#                               colData = sampleInfo,
#                               design = ~ group + seq_run)
# dds
```
...
directory <- "/Users/hollymcqueary/Dropbox/DosageCompensation/Indiv_Genes/HTseq/HTseq_update_May2018/FINAL_DATA_TO_USE_GC"
...
sampleInfo <- read.csv("/Users/hollymcqueary/Dropbox/DosageCompensation/Excel/sample_data_shrt.csv",header=TRUE)
...
```{r results and stuff, include=TRUE}
#################
#subset data set to do one comparison at a time 
colnames(ddsHTSeq)
ddsS1 <- ddsHTSeq[,c(7:9,46:48)]
ddsS1$condition <- droplevels(ddsS1$condition)
ddsS1$run <- droplevels(ddsS1$run)
colnames(ddsS1)
ddsS1 <- DESeq(ddsS1)
rowData(ddsS1)$dispersion
####find dispersion prior variance 
attr(dispersionFunction(ddsS1),"dispPriorVar")
#annotations
geneID <- rownames(ddsS1)
...
all(rownames(ddsS1) == genes$GENEID)
#need to remove the ones in rownames of dds that don't match genes$GENEID
matchedS1 <- intersect(rownames(ddsS1), genes$GENEID)
keep3 <- rownames(ddsS1) %in% matchedS1
ddsS1 <- ddsS1[keep3,]
length(ddsS1)
all(rownames(ddsS1) == genes$GENEID)
mcols(ddsS1) <- cbind(mcols(ddsS1), genes)

#mcols(ddsDef)
#######################################################################################
#sample 2 
colnames(ddsHTSeq)
ddsS2 <- ddsHTSeq[,c(10:12,46:48)]
ddsS2$condition <- droplevels(ddsS2$condition)
ddsS2$run <- droplevels(ddsS2$run)
colnames(ddsS2)
ddsS2 <- DESeq(ddsS2)
####find dispersion prior variance 
attr(dispersionFunction(ddsS2),"dispPriorVar")

#######################################################################################
#sample 3 
colnames(ddsHTSeq)
ddsS3 <- ddsHTSeq[,c(13:15,46:48)]
ddsS3$condition <- droplevels(ddsS3$condition)
ddsS3$run <- droplevels(ddsS3$run)
colnames(ddsS3)
ddsS3 <- DESeq(ddsS3)
####find dispersion prior variance 
attr(dispersionFunction(ddsS3),"dispPriorVar")

#######################################################################################
#sample 4 
colnames(ddsHTSeq)
ddsS4 <- ddsHTSeq[,c(19:21,46:48)]
ddsS4$condition <- droplevels(ddsS4$condition)
ddsS4$run <- droplevels(ddsS4$run)
colnames(ddsS4)
ddsS4 <- DESeq(ddsS4)
####find dispersion prior variance 
attr(dispersionFunction(ddsS4),"dispPriorVar")

#######################################################################################
#sample 5 
# colnames(ddsHTSeq)
# ddsS5 <- ddsHTSeq[,c(25:27,46:48)]
# ddsS5$condition <- droplevels(ddsS5$condition)
# ddsS5$run <- droplevels(ddsS5$run)
# colnames(ddsS5)
# ddsS5 <- estimateSizeFactors(ddsS5)
# View(ddsS5$run)
# View(ddsS5$condition)
# ddsS5 <- estimateDispersions(ddsS5)
# ddsS5 <- DESeq(ddsS5)

##### dds not working: error below 
...
#  combinations of the others and must be removed.
 ...
 # vignette('DESeq2')
####find dispersion prior variance 
# attr(dispersionFunction(ddsS5),"dispPriorVar")

#######################################################################################
#sample 7 
colnames(ddsHTSeq)

ddsS7 <- ddsHTSeq[,c(37:39,46:48)]
ddsS7$condition <- droplevels(ddsS7$condition)
ddsS7$run <- droplevels(ddsS7$run)
colnames(ddsS7)
ddsS7 <- DESeq(ddsS7)
####find dispersion prior variance 
attr(dispersionFunction(ddsS7),"dispPriorVar")

#######################################################################################
#sample 8 
colnames(ddsHTSeq)
ddsS8 <- ddsHTSeq[,c(40:42,46:48)]
ddsS8$condition <- droplevels(ddsS8$condition)
colnames(ddsS8)
ddsS8 <- DESeq(ddsS8)
####find dispersion prior variance 
attr(dispersionFunction(ddsS8),"dispPriorVar")

#######################################################################################
#sample 9 
colnames(ddsHTSeq)
ddsS9 <- ddsHTSeq[,c(43:45,46:48)]
ddsS9$condition <- droplevels(ddsS9$condition)
ddsS9$run <- droplevels(ddsS9$run)
colnames(ddsS9)
ddsS9 <- DESeq(ddsS9)
####find dispersion prior variance 
attr(dispersionFunction(ddsS9),"dispPriorVar")

#######################################################################################
#sample 18 
colnames(ddsHTSeq)
ddsS18 <- ddsHTSeq[,c(4:6,46:48)]
ddsS18$condition <- droplevels(ddsS18$condition)
ddsS18$run <- droplevels(ddsS18$run)
colnames(ddsS18)
ddsS18 <- DESeq(ddsS18)
####find dispersion prior variance 
attr(dispersionFunction(ddsS18),"dispPriorVar")

#######################################################################################
#sample 11 
colnames(ddsHTSeq)
ddsS11 <- ddsHTSeq[,c(1:3,46:48)]
ddsS11$condition <- droplevels(ddsS11$condition)
ddsS11$run <- droplevels(ddsS11$run)
colnames(ddsS11)
ddsS11 <- DESeq(ddsS11)
####find dispersion prior variance 
attr(dispersionFunction(ddsS11),"dispPriorVar")

#######################################################################################
#sample 49 
colnames(ddsHTSeq)
ddsS49 <- ddsHTSeq[,c(16:18,46:48)]
ddsS49$condition <- droplevels(ddsS49$condition)
ddsS49$run <- droplevels(ddsS49$run)
colnames(ddsS49)
ddsS49 <- DESeq(ddsS49)
####find dispersion prior variance 
attr(dispersionFunction(ddsS49),"dispPriorVar")

#######################################################################################
#sample 59 
colnames(ddsHTSeq)
ddsS59 <- ddsHTSeq[,c(22:24,46:48)]
ddsS59$condition <- droplevels(ddsS59$condition)
ddsS59$run <- droplevels(ddsS59$run)
colnames(ddsS59)
ddsS59 <- DESeq(ddsS59)
####find dispersion prior variance 
attr(dispersionFunction(ddsS59),"dispPriorVar")

#######################################################################################
#sample 61 
colnames(ddsHTSeq)
ddsS61 <- ddsHTSeq[,c(28:30,46:48)]
ddsS61$condition <- droplevels(ddsS61$condition)
ddsS61$run <- droplevels(ddsS61$run)
colnames(ddsS61)
ddsS61 <- DESeq(ddsS61)
####find dispersion prior variance 
attr(dispersionFunction(ddsS61),"dispPriorVar")

#######################################################################################
# #sample 69 
# colnames(ddsHTSeq)
# ddsS69 <- ddsHTSeq[,c(31:33,46:48)]
# ddsS69$condition <- droplevels(ddsS69$condition)
# ddsS69$run <- droplevels(ddsS69$run)
# colnames(ddsS69)
# ddsS69 <- DESeq(ddsS69)
# ####find dispersion prior variance 
# attr(dispersionFunction(ddsS69),"dispPriorVar")

#######################################################################################
#sample 76 
colnames(ddsHTSeq)
ddsS76 <- ddsHTSeq[,c(31:33,46:48)]
ddsS76$condition <- droplevels(ddsS76$condition)
ddsS76$run <- droplevels(ddsS76$run)
colnames(ddsS76)
ddsS76 <- DESeq(ddsS76)
####find dispersion prior variance 
attr(dispersionFunction(ddsS76),"dispPriorVar")

#######################################################################################
#sample 77 
colnames(ddsHTSeq)
ddsS77 <- ddsHTSeq[,c(34:36,46:48)]
ddsS77$condition <- droplevels(ddsS77$condition)
ddsS77$run <- droplevels(ddsS77$run)
colnames(ddsS77)
ddsS77 <- DESeq(ddsS77)
####find dispersion prior variance 
attr(dispersionFunction(ddsS77),"dispPriorVar")

########################################################################################
############# Results with subsetted dataset ###########################################
resS1 <- results(ddsS1, contrast=c("condition","Holly_1","Holly_Anc"))
summary(resS1)
hist(resS1$pvalue,col="pink", main="1 vs Anc, with aneuploid c'somes", xlab="p values")
sigGenes1 <- rownames(subset(resS1,padj<0.1))
s1FC <- resS1$log2FoldChange
s1FC <- as.data.frame(s1FC)
rownames(s1FC) <- rownames(resS1)
s1FC <- cat(s1FC,resS1$padj)
resS1$padj

View(s1FC)
write.csv(s1FC,file="s1FC.csv")
#line 2 
resS2 <- results(ddsS2, contrast=c("condition","Holly_2","Holly_Anc"))
summary(resS2)
hist(resS2$pvalue,col="pink", main="2 vs Anc, with aneuploid c'somes", xlab="p values")
sigGenes2 <- rownames(subset(resS2,padj<0.1))
#line 3 
resS3 <- results(ddsS3, contrast=c("condition","Holly_3","Holly_Anc"))#, cooksCutoff = FALSE)
summary(resS3)
sigGenes3 <- rownames(subset(resS3,padj<0.1))
hist(resS3$pvalue,col="pink", main="3 vs Anc, with aneuploid c'somes", xlab="p values")

#line 4 
resS4 <- results(ddsS4, contrast=c("condition","Holly_4","Holly_Anc"))#, cooksCutoff = FALSE)
summary(resS4)
hist(resS4$pvalue,col="pink", main="4 vs Anc, with aneuploid c'somes", xlab="p values")
sigGenes4 <- rownames(subset(resS4,padj<0.1))
#line 5 
resS5 <- results(ddsS5, contrast=c("condition","Holly_5","Holly_Anc"))#, cooksCutoff = FALSE)
summary(resS5)

#line 7 
res7Def <- results(ddsS7, contrast=c("condition","Holly_7","Holly_Anc"))#,cooksCutoff=FALSE)
summary(res7Def)
#line 8 
res8Def <- results(ddsS8, contrast=c("condition","Holly_8","Holly_Anc"))#, cooksCutoff = FALSE)
summary(res8Def)
#line 9 
res9Def <- results(ddsS9, contrast=c("condition","Holly_9","Holly_Anc"))#, cooksCutoff = FALSE)
summary(res9Def)
#line 11
res11Def <- results(ddsS11, contrast=c("condition","Holly_11","Holly_Anc"))#, cooksCutoff = FALSE)
summary(res11Def)
#line 18 
res18Def <- results(ddsS18, contrast=c("condition","Holly_18","Holly_Anc"))#, cooksCutoff = FALSE)
summary(res18Def)

#line 49 
res49Def <- results(ddsS49, contrast=c("condition","Holly_49","Holly_Anc"))#, cooksCutoff = FALSE)
summary(res49Def)
#line 59
res59Def <- results(ddsS59, contrast=c("condition","Holly_59","Holly_Anc"))#, cooksCutoff = FALSE)
summary(res59Def)
#line 61
res61Def <- results(ddsS61, contrast=c("condition","Holly_61","Holly_Anc"))#, cooksCutoff = FALSE)
summary(res61Def)
#line 69
#res69Def <- results(ddsS69, contrast=c("condition","Holly_69","Holly_Anc"))#, cooksCutoff = FALSE)
#summary(res69Def)
#line 76 
res76Def <- results(ddsS76, contrast=c("condition","Holly_76","Holly_Anc"))#, cooksCutoff = FALSE)
summary(res76Def)
#line 77
res77Def <- results(ddsS77, contrast=c("condition","Holly_77","Holly_Anc"))#, cooksCutoff = FALSE)
summary(res77Def)

#loop to put significant genes in array
...
ddsNoAneuInNorm <- estimateSizeFactors(ddsHTSeq, controlGenes=c(132:570,571:737,738:1542,1844:1975,3452:3790,4323:4812))
...
```{r common sig genes, Euploids, include=TRUE}
#Euploid lines 
eupLines <- c(1,2,3,5,9)
library(data.table)

res1Dat <- as.data.frame(res1)
write.csv(res1Dat, file="s1DEseqRes.csv")
setDT(res1Dat, keep.rownames = TRUE)[]
setnames(res1Dat, 1, "GENEID")

genes1DE <- subset(res1Dat,padj<0.1)

res2Dat <- as.data.frame(res2)
write.csv(res2Dat, file="s2DEseqRes.csv")
setDT(res2Dat, keep.rownames = TRUE)[]
setnames(res2Dat, 1, "GENEID")
genes2DE <- subset(res2Dat,padj<0.1)

res3Dat <- as.data.frame(res3)
write.csv(res3Dat, file="s3DEseqRes.csv")
setDT(res3Dat, keep.rownames = TRUE)[]
setnames(res3Dat, 1, "GENEID")
genes3DE <- subset(res3Dat,padj<0.1)

res5Dat <- as.data.frame(res5)
write.csv(res5Dat, file="s5DEseqRes.csv")
setDT(res5Dat, keep.rownames = TRUE)[]
setnames(res5Dat, 1, "GENEID")
genes5DE <- subset(res5Dat,padj<0.1)

res9Dat <- as.data.frame(res9)
write.csv(res9Dat, file="s9DEseqRes.csv")
setDT(res9Dat, keep.rownames = TRUE)[]
setnames(res9Dat, 1, "GENEID")
genes9DE <- subset(res9Dat,padj<0.1)

#find common DE genes between samples
#are there any commonly DE genes in all of the euploid lines?
test <- dplyr::inner_join(genes1DE,genes2DE, by="GENEID")
common12.3 <- inner_join(test,genes3DE, by="GENEID")
common123.5 <- inner_join(common12.3, genes5DE, by="GENEID")
common1235.9 <- inner_join(common123.5, genes9DE, by="GENEID")

#there are 326 commonly DE genes across 5 euploid lines 
...
write.csv(res4Dat, file="s4DEseqRes.csv")
...
chrI.all <- inner_join(chrI, genes11DE, by="GENEID")

chrV.all <- inner_join(genes4DE, genes49DE, by="GENEID")

chrVII.all <- inner_join(genes59DE, genes61DE, by="GENEID")

chrXII.all <- inner_join(genes18DE, genes77DE, by="GENEID")
