...
topDE.18 <- data.frame(topTags(et18))
#check to make sure it is a data frame
class(topDE.18)
#need to change the data frame into a data table so that you can compare with gene attr to get the gene names
topDE.18 <- data.table(topDE.18)
#this is how you make the rownames (where the gene names are) into an actual column so I can select by it. 
#make sure it has the right number of columns 
ncol(topDE.18)
#make it so that the rownames are now the tracking_id column
topDE.18$tracking_id <- rownames(topDE.18)
#compare your tracking_id column in pairwise comparison to the gene_id column in gene attr
topDE.18 = merge(topDE.18,geneattr,by.x="tracking_id",by.y="gene_id")
...
topDE.2 <- data.frame(topTags(et2))
class(topDE.2)
topDE.2 <- data.table(topDE.2)
ncol(topDE.2)
topDE.2$tracking_id <- rownames(topDE.2)
topDE.2 = merge(topDE.2,geneattr,by.x="tracking_id",by.y="gene_id")
ncol(topDE.2)
...
topTags(et3)
write.csv(topTags(et3),file="3vsAnc.edgeR.pairwise.csv")
...
topTags(et8)
write.csv(topTags(et8),file="8vsAnc.edgeR.pairwise.csv")
et9 <- exactTest(y,pair=c(30,1))
topTags(et9)
write.csv(topTags(et9),file="9vsAnc.edgeR.pairwise.csv")
et2 <- exactTest(y,pair=c(12,1))
topTags(et2)
write.csv(topTags(et2),file="2vsAnc.edgeR.pairwise.csv")
#MA Lines
