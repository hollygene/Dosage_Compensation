...
allGenes <- qplot(ratios$sampleOverAnc,geom="histogram", breaks=c(seq(0,6,by=0.05),max(ratios$sampleOverAnc)), xlab = "Ratio", ylab="Number of Genes",fill=I("turquoise"),col=I("darkturquoise"))+ geom_vline(xintercept = 1)+ coord_cartesian(xlim=c(0,6)) + geom_vline(aes(xintercept = mean(ratios$sampleOverAnc, na.rm = T)),
             colour = "red", linetype ="longdash", size = .5) + geom_vline(aes(xintercept = median(ratios$sampleOverAnc, na.rm = T)),
             colour = "blue", linetype ="longdash", size = .5) + theme_classic() + theme(axis.title = element_text(size=7),axis.text=element_text(size=7))
...
# View(ratios.All)
allRatiosPlot <- ggplot(ratios.All, aes(x=sampleOverAnc,fill=type)) +
  geom_histogram(breaks=c(seq(0, 6, by=0.05), max(ratios.All$sampleOverAnc)), position = "identity",alpha=0.6) + coord_cartesian(xlim=c(0,6),ylim=c(0,550)) + geom_vline(xintercept = 1) + 
...
ratioDistGC18 <- ratioDistributionsTrisomyMulti(ddsDefGC_18,genesGC_18,"chrI","chrXII", "A") + theme(axis.title.y = element_blank(),  axis.title.x = element_blank() )
...
ratioDistGC11 <- ratioDistributionsTriMonMulti(ddsDefGC_11,genesGC_11,"chrI","chrXV", "B") + theme(axis.title.y = element_blank(),  axis.title.x = element_blank() )

ratioDistMA108 <- ratioDistributionsTriMonMulti(ddsDefMAold_108,genesMAold_108,"chrVIII","chrIX", "") 

## for publication
pdf("gcMulti_samples_ratios.pdf")
gcMulti <- ggarrange(ratioDistGC18,ratioDistGC11,nrow=2,label.x="Ratio (Line/Ancestor)",label.y = "Number of Genes", common.legend = FALSE)
...
ratios76.All <- rbind(ratios.cis, ratios.cis2, ratios.cis3, ratios.Trans2)

ratioDistGC76 <- ggplot(ratios76.All, aes(x=sampleOverAnc,fill=type)) +
  geom_histogram(breaks=c(seq(0, 6, by=0.05), max(ratios76.All$sampleOverAnc)), position = "identity", alpha=0.3) + coord_cartesian(xlim=c(0,6),ylim=c(0,550)) + geom_vline(xintercept = 1) + 
  geom_vline(xintercept=1.5, col="magenta") + #exp for trisomy 
  geom_vline(xintercept=0.5,col="green") +   #exp for monosomy
  # geom_vline(xintercept=2,col="orange") + #exp for tetrasomy
  geom_vline(aes(xintercept = mean(ratios.cis$sampleOverAnc, na.rm = T)),colour = "red", linetype ="longdash", size = .5) +
  ...
  #mean trans genes
  theme_classic() + labs(x="Ratio (Line/Ancestor)",y="Number of Genes", title="C")
...
ratios.All <- rbind(ratios.cis, ratios.Cis2, ratios.Trans2)
allRatiosPlot <- ggplot(ratios.All, aes(x=sampleOverAnc,fill=type)) +
  geom_histogram(breaks=c(seq(0, 6, by=0.05), max(ratios.All$sampleOverAnc)), position = "identity", alpha=0.25) + coord_cartesian(xlim=c(0,6),ylim=c(0,550)) + geom_vline(xintercept = 1) + 
  geom_vline(xintercept=1.5, col="magenta") + #exp for trisomy 
  geom_vline(xintercept=0.5,col="green") +   #exp for monosomy
  # geom_vline(xintercept=2,col="orange") + #exp for tetrasomy
  geom_vline(aes(xintercept = mean(ratios.cis$sampleOverAnc, na.rm = T)),colour = "red", linetype ="longdash", size = .5) +
...
ratioDistGC8 <- ratioDistributionsTetrsomy(ddsDefGC_8,genesGC_8,"chrXVI","")
...
ratioDistMA29 <- ratioDistributionsMonosomyMAnew(ddsDefMAnewS29,genesMAnewS29,"chrIX","")
...
ggarrange(ratioDistMA15,ratioDistMA88,ratioDistMA119,ratioDistMA9,ncol=2,nrow=2,label.x="Ratio (Line/Ancestor)",label.y = "Number of Genes", common.legend = TRUE)
dev.off()
...
ratioDistGC76.IX <- ratioDistributionsTrisomyMulti2(ddsDefGC_76,genesGC_76,"chrIX","chrXIV","chrX", "Het Sample 76 Chr IX")
ratioDistGC76.X <- ratioDistributionsTrisomyMulti2(ddsDefGC_76,genesGC_76,"chrX","chrXIV","chrIX", "Het Sample 76 Chr IX")
ratioDistGC76.XIV <- ratioDistributionsTrisomyMulti2(ddsDefGC_76,genesGC_76,"chrXIV","chrIX","chrX", "Het Sample 76 Chr XIV")
...
esrGenesHet <- cbind(esrGenesMA152,esrGenesGC11,esrGenesGC18,esrGenesGC7,esrGenesGC4,esrGenesGC49,esrGenesGC59c,esrGenesGC61,esrGenesGC76,esrGenesGC77,esrGenesGC8)

##trial run of heatmap 
# want genes on y axis 
# sample clustered by ancestor and ploidy on x axis
# values based on log2foldchange 
library(ggplot2)
