...
#colChrm <- colorRampPalette(brewer.pal(16,"Set1"))(nchrm)
...
#lines <- list(c(1:3),c(4:6),c(7:9),c(10:12),c(13:15))
#lines.GC <- as.factor(c("11","18","1","21","2","31","3","49","4","59","5","61","66","68","6","76","77","7","8","9","A"))

#lcpmGC <-cpm(x.GC, log=TRUE)
#lcpmGC$chrm <- 
#  str(x.GC)
#x.GC$data.frame$TXCHROM
#nrow(lcpmGC)
#rm(i)
#cpmGC[,grep(("11"),colnames(cpmGC))]
#var <- matrix(nrow=3291,ncol=61)
#var 
#plot(density(lcpmGC[,1]),col=colGC[1],lwd=2,ylim=c(0,0.33),las=2,main="",xlab="")
#title(main="A. Raw Data", xlab="Log-cpm")
#abline(v=0,lty=3)
#var <- as.matrix(apply(cpmGC[,grep((i),colnames(cpmGC))],1,var),byrow=FALSE) 
#magic_for(cbind)
#magic_free()
#i=2
#for(i in 2:lines.GC) { 
#  var <- as.matrix(apply(cpmGC[,grep((i),colnames(cpmGC))],1,var),byrow=FALSE)
 # var[i] <- (apply(cpmGC[,grep((i),colnames(cpmGC))],1,var))
  #varAll <- rbind(var,var[i])
#  for (i in 2:lines.GC) { 
 #   magic_for(data.frame)
#    data.frame(var,var[i])
#  }
#}
#apply(cpmGC[,grep(("11"),colnames(cpmGC))],1,var)
#magic_result_as_dataframe()
#n.rows <- nrow(cpmGC)
#n.rows
#out <- matrix(NA, nrow=n.rows, ncol=ncol(cpmGC))
#out
#for (i in 2:lines.GC) {
#  var <- as.matrix(apply(cpmGC[,grep((i),colnames(cpmGC))],1,var),byrow=FALSE)
#  out[i,] <- (apply(cpmGC[,grep((i),colnames(cpmGC))],1,var))
#}
#str(lines.GC)
#View(rep.MA)
#for(line in rep.MA){
...
#var2 <- as.matrix(apply(cpmGC[,grep((1),colnames(cpmGC))],1,var),byrow=FALSE)
#for (i in 2:length(lines.GC)) {
#  v <- as.matrix(apply(cpmGC[,grep((i),colnames(cpmGC))],1,var))
#  var2[i] <- v
#  varAll <- cbind(var2,var2[i])
#}
#rm(i)
#for (i in 2:length(lines.GC)) {
#  var <- as.matrix(apply(cpmGC[,grep((1),colnames(cpmGC))],1,var),byrow=FALSE)
#  v <- (apply(cpmGC[,grep((i),colnames(cpmGC))],1,var))
#  test.out <- cbind(L11=var, L[i]=v)
#  return(test.out)
#}
#i=2
...
View(lcpmGC)
 ...
 #ind <- grep(as.character(lines.GC["11"]),names)
#  cols <- lcpmGC[,ind]
...
i=1
rm(i)
v <- matrix(data=NA,nrow=nrow(lcpmGC),ncol=length(lines.GC))
is.list(v)
...
mycols2 <- sample(cl, 16)
#colChrm <- palette(rainbow(16))
#mean[,1] <- as.character(mean[,1])
#nlevels(mean[,1])
#v <- data.frame(v)
#na.omit(v)
#na.omit(mean)
#testPal <- colorRampPalette()
#is.list(mean)
#mean <- unlist(mean)
#v[,1] <- testPal(16)[as.numeric(cut(v[,1],breaks=16))]
#is.list(v)
#v <- unlist(v)
#mean <- unlist(mean)
#v <- unlist(v)

plot(mean,v,col=mycols2,log="y")
legend <- col
{plot(mean,cv,log="y",col=factor(mean[,1]))
  
legend("topright", legend = legend,col=factor(mean[,1]))}
dim(cv)
plot(mean,v,col=factor(mean[,1]))

#,legend("topright",chrm,text.col=colChrm,bty="n",cex=0.4,ncol=4)

par(mfrow=c(1,1))

nlevels(colChrm)
{pdf("test1.pdf")
plot(mean,cv,col=factor(mean[,1]))
}


### same thing but not log transformed
#v.c <- matrix(data=NA,nrow=nrow(cpmGC),ncol=length(lines.GC))
...
#mean.c <- matrix(data=NA,nrow=nrow(cpmGC),ncol=length(lines.GC))
#for (i in 1:length(lines.GC)) {
#  cpm2 <- cpmGC[,1:60]
#  ind <- grep(as.character(lines.GC[i]),names)
#  cols <- cpmGC[,ind]
...
#std.c <- matrix(data=NA,nrow=nrow(cpmGC),ncol=length(lines.GC))
#for (i in 1:length(lines.GC)) {
#  cpm2 <- cpmGC[,1:60]
#  ind <- grep(as.character(lines.GC[i]),names)
#  cols <- cpmGC[,ind]
...
#plot(mean.c,cv.c,col=colGC)

#plot(mean.c,v.c,col=colGC)
#colnames(v) <- lines.GC
#View(v)
#v <- data.frame(v)
...
for (i in 1:21) {
  ...
  title(main=paste("Distribution of Variance",lines.GC[i]),xlab="var of lcpm")
  abline(v=0,lty=3)
  dev.off()
}

#make colnames into the proper column names
colnames(v) <-  c("11","18","1","21","2","31","3","49","4","59","5","61","66","68","6","76","77","7","8","9","A")
str(v)
# all on the same graph 
{
pdf("Dis_variance.pdf")
plot(density(v[,1]),col=colGC[1],lwd=2,las=2,main="",xlab="")
title(main="Variance", xlab="variance")
abline(v=0, lty=3)
for (i in 2:length(lines.GC)) {
  den <- density(v[,i])
  lines(den$x,den$y,col=colGC[i],lwd=2)
...
#rm(den)
#View(cols)
#lcpmGC[,grep(("[21]"),colnames(lcpmGC))]
#fix 
#pdf("example.pdf")
#var2 <- as.matrix(apply(lcpmGC[,grep((lines.GC[1]),colnames(lcpmGC))],1,var),byrow=FALSE)
#  plot(density(var2[,1]),col=colGC[1],lwd=2,las=2,main="",xlab="")
#v <- as.matrix(apply(lcpmGC[,grep((lines.GC[2]),colnames(lcpmGC))],1,var))
#  den <- density(v[,1])
#  lines(den$x,den$y,col=colGC[2],lwd=2)
#lines.GC
#rm(v[i])
#length(v)
#length(var2)
#length((apply(lcpmGC[,grep((2),colnames(lcpmGC))],1,var)))
#histogram of distribution of variance
#hist(var2)
#for (i in 2:(lines.GC)) {
#  var2 <- cbind(var2,(apply(cpmGC[,grep((i),colnames(cpmGC))],1,var)))
#  return(var2)
#}
#var <- as.matrix(apply(cpmGC[,grep((1),colnames(cpmGC))],1,var),byrow=FALSE)
#  test.out <- cbind(L11=var, "l"(i)=(apply(cpmGC[,grep((i),colnames(cpmGC))],1,var)))
#  return(test.out)
#remove unwanted rows from cpmGC data frame
#attempt at aggregate
#by = list()
#class(cpmGC)
#cpmGC <- data.frame(cpmGC)
#test1 <- aggregate(cpmGC,by = list(cpmGC$HTseq11A,cpmGC$HTseq11B,cpmGC$HTseq11C),var)
#names <- colnames(cpmGC)
#dim(v)
#dim(var2)
#rm(var2)
#rm(varAll)
#varAll <- matrix(varAll,nrow=3292,ncol=61)
#calcVar <- function (i) {
#  var <- as.matrix(apply(cpmGC[,grep((1),colnames(cpmGC))],1,var),byrow=FALSE)
#  v <- (apply(cpmGC[,grep((i),colnames(cpmGC))],1,var))
#  test.out <- cbind(var, v)
#  return(test.out)
#}
#test.out <- t(tapply(1:length(lines.GC), FUN=calcVar))
#str(cpmGC)
#test.out <- sapply(1:length(lines.GC), function(n)
#  (apply(cpmGC[,grep((n),colnames(cpmGC))],1,var)))
#magic_for(print, silent=TRUE)
#for (i in 1:length(lines.GC)) {
#  var <- as.matrix(apply(cpmGC[,grep((i),colnames(cpmGC))],1,var))
#  print(var)
#}
#magic_result_as_dataframe()
#l1_var <- apply(cpmGC[,1:3], 1, var)
#l1_var
#l1_mean <- apply(cpmGC[,1:3],1,mean)
#l1_mean
#correlation of mean and variance
#cor(l1_mean,l1_var,method="spearman")
#plot(log2(l1_mean),log2(l1_var),pch=".")
#plot(l1_var,l1_mean)
#plot(density(l1_var),pch=".")
#class(cpmGC)
#str(cpmGC)
#samplesTest <- colnames(cpmGC)
#samplesTest
...
contr.matrix <- makeContrasts(
  X1vsGCA = group.GC1-group.GCAnc,
  X11vsGCA = group.GC11-group.GCAnc,
  X18vsGCA = group.GC18-group.GCAnc,
  X2vsGCA = group.GC2-group.GCAnc,
  X21vsGCA = group.GC21-group.GCAnc,
  X3vsGCA = group.GC3-group.GCAnc,
  X31vsGCA = group.GC31-group.GCAnc,
  X4vsGCA = group.GC4-group.GCAnc,
  X49vsGCA = group.GC49-group.GCAnc,
  X5vsGCA = group.GC5-group.GCAnc,
  X59vsGCA = group.GC59-group.GCAnc,
  X6vsGCA = group.GC6-group.GCAnc,
  X61vsGCA = group.GC61-group.GCAnc,
  X66vsGCA = group.GC66-group.GCAnc,
  X68vsGCA = group.GC68-group.GCAnc,
  X7vsGCA = group.GC7-group.GCAnc,
  X76vsGCA = group.GC76-group.GCAnc,
  X77vsGCA = group.GC77-group.GCAnc,
  X8vsGCA = group.GC8-group.GCAnc,
  X9vsGCA = group.GC9-group.GCAnc,
