...
library(moments)
##########read the data
sc_counts <- read.csv("/Users/hollymcqueary/Dropbox/McQueary/Dosage-Compensation/Redo/New_Again/genes.fpkm_table.csv",header=T)
...
index1 <- which(sc_anc[,1]=="I")
index2 <- which(sc_anc[,1]=="II")
index3 <- which(sc_anc[,1]=="III")
index4 <- which(sc_anc[,1]=="IV")
index5 <- which(sc_anc[,1]=="V")
index6 <- which(sc_anc[,1]=="VI")
index7 <- which(sc_anc[,1]=="VII")
index8 <- which(sc_anc[,1]=="VIII")
index9 <- which(sc_anc[,1]=="IX")
index10 <- which(sc_anc[,1]=="X")
index11 <- which(sc_anc[,1]=="XI")
index12 <- which(sc_anc[,1]=="XII")
index13 <- which(sc_anc[,1]=="XIII")
index14 <- which(sc_anc[,1]=="XIV")
index15 <- which(sc_anc[,1]=="XV")
index16 <- which(sc_anc[,1]=="XVI")
