...
```{r other files,eval=TRUE,include=TRUE}
esrGenes <- read.csv("/Users/hollymcqueary/Dropbox/McQueary/Dosage-Compensation/Paper_Results/ESR_genes/ESR_genes_Gasch2000.csv",header=FALSE)
as.data.frame(esrGenes)
names(esrGenes)[1] <- "GENEID"

dosageSensitiveGenes <- read.csv("/Users/hollymcqueary/Dropbox/McQueary/Dosage-Compensation/Indiv_Genes/DENonDEGenes/dosage_sensitive_genes.csv")
as.data.frame(dosageSensitiveGenes)
names(dosageSensitiveGenes)[1] <- "GENEID"

ASRgenes <- read.table("/Users/hollymcqueary/Dropbox/McQueary/Dosage-Compensation/Paper_Results/ESR_genes/ASRGenes.txt",header=TRUE)
...
```{r relevel,eval=TRUE,include=TRUE}
...
```{r prefilter,eval=TRUE,include=TRUE}
...
# ddsHTSeqMAoldtest2 <- ddsHTSeqMAoldtest2[keepGC,]
# ddsHTSeqMAoldtest2
...
# ddsMAnew29 <- ddsHTSeqMAnewS29[keepMAnew,]
# length(ddsMAnew29)
...
# # ddsHTSeqMAnewS29 <- ddsHTSeqMAnewS29[keepGC,]
...
#now need to make sure all the GC and old MA samples match the new MA samples since they got rid of a couple 
...
# keep5 <- rownames(ddsHTSeqGC_77)
# maold <- rownames(ddsHTSeqMAold_108)
# keep4 <- maold %in% maold
# summary(keep4)
...
##### Function I found to make an outlier bin 
...
#                            binwidth = NULL) {
#   
#   printing_min_max <- x %>% summarise(sprintf("round(min(%s, na.rm = TRUE), 1)", var_name),
...
#                            "1 == 1")
#   
#   x_regular <- x %>% filter(ceiling_filter, floor_filter) %>% 
#     select_(var_name)
#   
#   x_to_roll_ceiling <- x %>% filter(
#     sprintf("%s >= %f", var_name, cut_off_ceiling)) %>% select(var_name)
#   if (!is.na(cut_off_ceiling)) x_to_roll_ceiling[, 1] <- cut_off_ceiling
#   
#   x_to_roll_floor <- x %>% filter(
#     sprintf("%s <= %f", var_name, cut_off_floor)) %>% select(var_name)
...
# ratioDistributionsEuploids(ddsDefMAnewS50,genesMAnewS50,"Sample_50_MA")
...
# determine if any significantly differentially expressed genes match those identified by gasch et al 2000
  esrRes <- list()
  esrRes$Sig <- dplyr::inner_join(esrGenes,resDiSig,by=c("GENEID","GENEID"))
  esrRes$NonSig <- dplyr::inner_join(esrGenes,resDiNonSig,by=c("GENEID","GENEID"))
  return(esrRes)
}
...
```{r sumRes fdrtool,include=FALSE}
...
```{r MAidentify, eval=FALSE,include=FALSE}
# {plotMA(resLFC2, ylim=c(-2,2))
...
```{r fig.width=8, fig.height=3,include=FALSE,eval=FALSE}
...
``` {r reportingTools,include=FALSE}
#source("https://bioconductor.org/biocLite.R")
...
```{r copyMultifactor,include=FALSE}
# ddsMF <- dds
```
...
```{r multiTypeResults,include=FALSE}
...
```{r figHeatmapSamples, fig.height=4, fig.width=6,include=FALSE}
...
```{r WaldTest, eval=FALSE,include=FALSE}
...
```{r simpleContrast, eval=FALSE,include=FALSE}
...
```{r combineFactors, eval=FALSE,include=FALSE}
...
```{r interFig, echo=FALSE, results="hide", fig.height=3,include=FALSE}
# npg <- 20
...
```{r interFig2, echo=FALSE, results="hide", fig.height=3,include=FALSE}
# mu[4] <- 2^12
# mu[6] <- 2^8
# counts <- rnbinom(6*npg, mu=rep(mu,each=npg), size=1/.01)
...
```{r simpleLRT, eval=FALSE,include=FALSE}
...
```{r simpleLRT2, eval=FALSE,include=FALSE}
...
```{r normFactors, eval=FALSE,include=FALSE}
...
```{r offsetTransform, eval=FALSE,include=FALSE}
...
```{r lineardep, echo=FALSE,include=FALSE}
...
```{r lineardep2, echo=FALSE,include=FALSE}
...
```{r lineardep3, echo=FALSE,include=FALSE}
...
```{r groupeffect4, eval=FALSE,include=FALSE}
...
```{r fighistindepfilt,include=FALSE}
...
```{r vanillaDESeq, eval=FALSE,include=FALSE}
