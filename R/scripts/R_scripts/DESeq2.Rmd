...
directoryGC.fil <- "/Users/hollymcqueary/Dropbox/McQueary/Dosage-Compensation/Indiv_Genes/HTseq/HTseq_update_May2018/GC/exons_unstranded/filtered_genes"
...
```{r htseqInput}
sampleFilesGC <- list.files(directoryGC)
...
ddsHTSeqGC <- DESeqDataSetFromHTSeqCount(sampleTable = sampleTableGC,
                                       directory = directoryGC,
                                       design= ~ condition + run)

ddsHTSeqGC

write.csv(counts(ddsHTSeqGC),file="counts.DEseqGC.csv")
#filtered genes 
...
subsetS5 <- which(ddsHTSeqGC.fil$condition=="Holly_5")
subsetAnc <- which(ddsHTSeqGC.fil$condition=="Holly_Anc")
subsetS5 <- c(subsetS5,subsetAnc)
ddsHTSeqGCS5 <- ddsHTSeqGC.fil[,subsetS5]
ddsHTSeqGCS5$condition
ddsHTSeqGCS5$condition <- droplevels(ddsHTSeqGCS5$condition)
ddsHTSeqGCS5$condition

subsetS8 <- which(ddsHTSeqGC.fil$condition=="Holly_8")
subsetS8 <- c(subsetS8,subsetAnc)
ddsHTSeqGCS8 <- ddsHTSeqGC.fil[,subsetS8]
...
subsetS1 <- which(ddsHTSeqGC.fil$condition=="Holly_1")
subsetS2 <- which(ddsHTSeqGC.fil$condition=="Holly_2")
subsetS3 <- which(ddsHTSeqGC.fil$condition=="Holly_3")
subsetS4 <- which(ddsHTSeqGC.fil$condition=="Holly_4")
subsetS7 <- which(ddsHTSeqGC.fil$condition=="Holly_7")
subsetS18 <- which(ddsHTSeqGC.fil$condition=="Holly_18")
subsetS49 <- which(ddsHTSeqGC.fil$condition=="Holly_49")
subsetS59 <- which(ddsHTSeqGC.fil$condition=="Holly_59")
subsetS61 <- which(ddsHTSeqGC.fil$condition=="Holly_61")
subsetS76 <- which(ddsHTSeqGC.fil$condition=="Holly_76")
subsetS77 <- which(ddsHTSeqGC.fil$condition=="Holly_77")
...
ddsHTSeqGC.short <- ddsHTSeqGC.fil[,subsetAll]
...
vsdGC.fil <- vst(ddsHTSeqGC.fil,blind=FALSE)
rldGC.fil <- rlog(ddsHTSeqGC.fil,blind=FALSE)
head(assay(vsdGC.fil),3)
plotPCA(vsdGC.fil, intgroup=c("condition"))
pcaDataGC.fil <- plotPCA(vsdGC.fil,intgroup=c("condition"),returnData=TRUE)
percentVar <- round(100 * attr(pcaDataGC.fil,"percentVar"))
library(ggplot2)
ggplot(pcaDataGC.fil,aes(PC1,PC2,shape=condition)) +
  geom_point(size=3) +
  ...
  colData(ddsHTSeqGC)
plotPCA(vsdGC.fil, intgroup=c("run"))
pcaDataGC.fil <- plotPCA(vsdGC.fil,intgroup=c("run"),returnData=TRUE)
percentVar <- round(100 * attr(pcaDataGC.fil,"percentVar"))
library(ggplot2)
ggplot(pcaDataGC.fil,aes(PC1,PC2,color=run)) +
  geom_point(size=3) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) +
  scale_fill_hue(l=80) 
...
```{r prefilter}
keepGC <- rowSums(counts(ddsHTSeqGC)) >= 20
ddsHTSeqGC <- ddsHTSeqGC[keepGC,]
ddsHTSeqGC
counts(ddsHTSeqGC)
dim(counts(ddsHTSeqGC))

#make a separate data frame with just the ancestor replicates
ancGC <-  counts(ddsHTSeqGC)[,46:48]
...
ddsHTSeqGC <- ddsHTSeqGCtest
## get rid of any NAs
ddsHTSeqGC <- na.omit(ddsHTSeqGC)
dim(ddsHTSeqGC)
