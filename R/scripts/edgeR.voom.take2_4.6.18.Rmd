...
files.MA <- list.files(path="/Users/hollymcqueary/Dropbox/McQueary/Dosage-Compensation/Indiv_Genes/HTseq/HTseq_update_May2018/MA_old/exons_unstranded",pattern = "*.txt", full.names = TRUE)
files.MA
...
#samplenamesGC <- c("1A","11A","11B","11C","18A","18B","18C","1B","1C","21A","21C","2A","2B","2C","31A","31B","31C","3A","3B","3C","4A","49A","49B","49C","4B","4C","59A","59B","59C","5A","5B","5C","61A","61B","61C","68A","68B","68C","6A","6B","6C","7A","76A","76B","76C","77A","77B","77C","7B","7C","8A","8B","8C","9A","9B","9C","GCAncA","GCAncB","GCAncC")

#group.GC <- as.factor(c("1","11","11","11","18","18","18","1","1","21","21","2","2","2","31","31","31","3","3","3","4","49","49","49","4","4","59","59","59","5","5","5","61","61","61","68","68","68","6","6","6","7","76","76","76","77","77","77","7","7","8","8","8","9","9","9","GCAnc","GCAnc","GCAnc"))
...
write.csv(x.MA$counts,file="raw_counts_oldMA.csv")
...
genesMA <- ensembldb::select(txdb, keys=geneidMA, columnns="GENEID","TXCHROM" , keytype="GENEID")
#genelengthsGC <- 
#genesGC
...
genesMA.test <- genesMA[-(grep(paste0(remove1,"$"),genesMA$TXCHROM,perl=TRUE)),]
genesMA.test <- na.omit(genesMA.test)
genesMA.test
genesMA <- genesMA.test

lengths <- transcriptLengths(txdb)

genesMA <- merge(genesMA,lengths,by.x="GENEID",by.y="gene_id")
...
rownames(x.MA$counts)
x.MA$genes$GENEID
length(x.MA$genes$GENEID)
length(rownames(x.MA$counts))
#testing to remove the genes I don't want 
counts <- x.MA$counts
(rownames(counts))%in%genesMA$GENEID
test <- subset(counts,(rownames(counts))%in%genesMA$GENEID)
#works!
x.MA$counts <- test
View(x.MA$counts)
#x.GC.test <- x.GC
...
cpmMA <- cpm(x.MA,normalized.lib.sizes = FALSE, prior.count = 0.5)
lcpmMA <- cpm(x.MA,normalized.lib.sizes = FALSE, prior.count = 0.5, log=TRUE)
rpkmMA <- rpkm(x.MA,x.MA$genes$tx_len, normalized.lib.sizes = FALSE, prior.count = 0.5)
write.csv(cpmMA,file="cpm_MAold.csv")
...
table(rowSums(cpmMA>0.0001)>=3)
table(rowSums(cpmMA>99999)<=1 & rowSums(cpmMA>0)>=3)
keep.exprsMA <- rowSums(cpmMA>99999)<=1 & rowSums(cpmMA>0)>=3
#keep.exprsGC.test <- rowSums(cpmGC>1.1)>=3
#str(keep.exprsGC)
#str(x.GC)
...
colMA <- colorRampPalette(brewer.pal(48,"Set1"))(ncol(x.MA))
nlevels(colMA)
{
  pdf("Raw_dataMA_cpm.pdf")
plot(density(cpmMA[,1]),col=colMA[1],lwd=2,las=2,main="",xlab="")
...
legend("topright",samplenamesMA, text.col=colMA, bty="n",cex=0.4,ncol=4)
}


{
pdf("Filtered_dataMA_cpm.pdf")
cpmMAfil<- cpm(x.MA)
plot(density(cpmMAfil[,1]),na.rm=TRUE, col=colMA[1],lwd=2,las=2,main="",xlab="")
title(main="B. Filtered Data", xlab="cpm")
abline(v=0, lty=3)
for (i in 2:nsamplesMA) {
  den <- density(cpmMAfil[,i])
  lines(den$x,den$y,col=colMA[i],lwd=2)
}
legend("topright",samplenamesMA, text.col=colMA,bty="n",cex=0.4,ncol=4)
}
...
#line 21
et18 <- exactTest(x.GC.des, pair=c("GCAnc","18"))
...
#line 4
View(et4.test$table)
et4.test <- exactTest(x.GC.des, pair=c("GCAnc","66"))
...
s1vAnc$X2 = p.adjust(diff_exp_1$table$PValue, method = "BH") #Benjamini–Hochberg procedure
