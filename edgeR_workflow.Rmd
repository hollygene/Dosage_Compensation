...
colnames(design) <- gsub("group","exp",colnames(design))
design
...
efit <- eBayes(vfit)
plotSA(efit)
#estimate number of DE genes
summary(decideTests(efit))
#need to look up what reat does
tfit <- treat(vfit,lfc=1)
dt <- decideTests(tfit)
de.common1.11 <- which(dt[,1]!=0 & dt[,2]!=0)
length(de.common1.11)
```
...
group
X1vsGCA <- makeContrasts(X1-GCA,levels=design.2)
lrt.1vsGCA <- glmLRT(fit,contrast=X1vsGCA)
topTags(lrt.1vsGCA)
#to do the QL method
fit <- glmQLFit(y,design.2)
qlf.1vsGCA <- glmQLFTest(fit,contrast=X1vsGCA)
topTags(qlf.1vsGCA)
```
