...
lcpmGC <- merge(genes2,lcpmGC, by.x="genes", by.y="x.GC.genes.GENEID")
lcpmGC <- lcpmGC[,-2]
lcpmGC <- lcpmGC[,-3]
...
mean <- cbind(lcpmGC$genes,lcpmGC$x.GC.genes.TXCHROM.x,mean)
...
std <- cbind(lcpmGC$genes,lcpmGC$x.GC.genes.TXCHROM.x,std)
...
cv <- cbind(lcpmGC$genes,lcpmGC$x.GC.genes.TXCHROM.x,cv)
...
for (i in 3:23) {
  ...
  print(ggplot () + geom_point(aes(x=mean[,i],y=vari[,i],col=factor(mean[,2]))) + scale_y_log10() + labs(title=paste("Mean-Variance Relationship Line",line.G[i]), y="variance",x="mean"))
  dev.off()
}
...
for (i in 3:23) {
  ...
  plot(density(vari[,i]),col=colGC[i],lwd=2,las=2,main="",xlab="")
