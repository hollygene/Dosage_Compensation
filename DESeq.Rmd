...
GC.table <- data.frame(sampleName = samplenamesGC,fileName = filenames.GC, condition = group.GC)
View(GC.table)
...
desGC <- DESeqDataSetFromHTSeqCount(sampleTable = GC.table, directory = "/Users/hollymcqueary/Dropbox/McQueary/Dosage-Compensation/Indiv_Genes/HTseq/HTseq_update_May2018/GC", design = ~ condition)
...
res49.loc
sum(res49.loc$padj <0.05, na.rm=TRUE)
...
plotDispEsts(desGC.loc)
lines.GC <-  as.factor(c("11","18","1", "21","2", "31","3", "49","4", "59", "5", "61","66","68", "6", "76","77","7", "8", "9","GCAnc"))
...
rm(i)
for (i in 1:20) {
  line <- nbinomTest(ddsGC,"GCAnc",lines.GC[i])
    pdf(paste("MA_plot_Line",lines.GC[i],".pdf",sep=""))
  plotMA(line,main=paste("MA Plot Line",lines.GC[i],sep=" "))
    pdf(paste("hist_pval_Line",lines.GC[i],".pdf",sep=""))
  hist(line$padj,breaks=100,col="turquoise",border="turquoise",main=paste("Distribution of P-Values Line",lines.GC[i],sep=" "))
  abline(v=0.05)
  write.csv(line, file=paste("DE_results",lines.GC[i],".csv",sep="_"))
  dev.off()
}
