...
group.GC <- as.factor(c("11","11","11","18","18","18","1","1","1","21","21","2","2","2","31","31","31","3","3","3","49","49","49","4","4","4","59","59","5","5","5","61","61","61","66","66","66","68","68","68","6","6","6","76","76","76","77","77","77","7","7","7","8","8","8","9","9","9","Anc","Anc","Anc"))
length(group.GC)
levels(group.GC)
nlevels(group.GC)
...
dim(x.GC)
table(rowSums(x.GC$counts==0)==61)
...
dontkeep.exprGC <- rowSums(x.GC$counts==0)==61
x.GCFil <- x.GC[!dontkeep.exprGC,,keep.lib.sizes=FALSE]
dim(x.GCFil)
#remove genes that are lowly expressed in all samples (ie not expressed in any)
#dim(x.MA)
...
#colGC <- brewer.pal(nsamplesGC,"Set2")
colGC <- colorRampPalette(brewer.pal(8,"Set1"))(21)
{
  pdf("Raw_dataGC.pdf")
...
lcpmGCfil <- cpm(x.GCFil, log=TRUE)
plot(density(lcpmGCfil[,1]),col=colGC[1],lwd=2,ylim=c(0,0.33),las=2,main="",xlab="")
...
{
pdf("Filtered_dataGC2.pdf")
lcpmGCfil2 <- cpm(x.GC.str, log=TRUE)
...
lcpmGC
#cpmGC <- cpm(x.GC)
remove <- c("__no_feature", "__ambiguous","__alignment_not_unique")
...
den <- matrix(data=NA,nrow=nrow(lcpmGC),ncol=length(lines.GC))
