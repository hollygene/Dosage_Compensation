...
#i=2
lcpmGC
var2 <- as.matrix(apply(lcpmGC[,grep((2),colnames(lcpmGC))],1,var),byrow=FALSE)
rm(i)
i=2

{
  pdf("Dist_Variance.pdf")
  plot(density(var2[,1]),col=colGC[1],lwd=2,las=2,main="",xlab="")
  title(main="Dist_Variance",xlab="var of lcpm")
  abline(v=0,lty=3)
for (i in 2:length(lines.GC)) {
  v[i] <- as.matrix(apply(lcpmGC[,grep((i),colnames(lcpmGC))],1,var))
  den[i] <- density(v[,1])
  lines(den[i],col=colGC[i],lwd=2)
}
}

var2 <- as.matrix(apply(lcpmGC[,grep((1),colnames(lcpmGC))],1,var),byrow=FALSE)
  plot(density(var2[,1]),col=colGC[2],lwd=2,las=2,main="",xlab="")
v <- as.matrix(apply(lcpmGC[,grep((2),colnames(lcpmGC))],1,var))
  den <- density(v[,1])
...
class(var2)
colnames(var2[i]) <- paste("line",i,sep="")
class(lines.GC)
