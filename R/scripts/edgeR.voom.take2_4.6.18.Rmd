...
files.GC <- list.files(path="/Users/hollymcqueary/Dropbox/McQueary/Dosage-Compensation/Indiv_Genes/HTseq/HTseq_update_May2018/GC/exons_unstranded",pattern = "*.txt", full.names = TRUE)
files.GC
...
x.GC$samples
samplenamesGC <- c("1A","11A","11B","11C","18A","18B","18C","1B","1C","2A","2B","2C","3A","3B","3C","4A","49A","49B","49C","4B","4C","59A","59B","59C","5A","5B","5C","61A","61B","61C","7A","76A","76B","76C","77A","77B","77C","7B","7C","8A","8B","8C","9A","9B","9C","GCAncA","GCAncB","GCAncC")

group.GC <- as.factor(c("1","11","11","11","18","18","18","1","1","2","2","2","3","3","3","4","49","49","49","4","4","59","59","59","5","5","5","61","61","61","7","76","76","76","77","77","77","7","7","8","8","8","9","9","9","GCAnc","GCAnc","GCAnc"))
...
cpmMANew <- data.frame(names=row.names(cpmMANew),cpmMANew)
cpmMANew.test <- merge(genesMANew,cpmMANew, by.x="GENEID", by.y="names")
cpmMANew <- cpmMANew.test
# table(rowSums(cpmMANew>0.0001)>=3)
# table(rowSums(cpmMANew>99999)<=1 & rowSums(cpmMANew>0)>=3)
# keep.exprsMANew <- rowSums(cpmMANew>99999)<=1 & rowSums(cpmMANew>0)>=3
#keep.exprsGC.test <- rowSums(cpmGC>1.1)>=3
keepTest <- cpmMANew$AncB>1 & cpmMANew$AncA>1 & cpmMANew$AncC>1
summary(keepTest)
#str(keep.exprsGC)
#str(x.GC)
# x.MANew <- x.MANew[keep.exprsMANew,keep.lib.sizes=FALSE]
# cpmMANew2 <- cpm(x.MANew)
x.MANew.test <- x.MANew[keepTest,keep.lib.sizes=FALSE]
x.MANew.test$counts
#x.GC.test <- x.GC[keep.exprsGC.test,keep.lib.sizes=FALSE]
dim(x.MANew)
#table(rowSums(x.GC.str$counts>0.0001))
#make x.GC the filtered one 
x.MANew <- na.omit(x.MANew)

dim(x.MANew)

table(rowSums(cpmMAOld>0.0001)>=3)
table(rowSums(cpmMAOld>99999)<=1 & rowSums(cpmMAOld>0)>=3)
keep.exprsMAOld <- rowSums(cpmMAOld>99999)<=1 & rowSums(cpmMAOld>0)>=3
#keep.exprsGC.test <- rowSums(cpmGC>1.1)>=3
#str(keep.exprsGC)
#str(x.GC)
x.MAOld <- x.MAOld[keep.exprsMAOld,keep.lib.sizes=FALSE]
#x.GC.test <- x.GC[keep.exprsGC.test,keep.lib.sizes=FALSE]
dim(x.MAOld)
#table(rowSums(x.GC.str$counts>0.0001))
#make x.GC the filtered one 
x.MAOld <- na.omit(x.MAOld)
...
x.normMANew <- calcNormFactors(x.MANew, method="TMM")
x.normMANew$samples
x.normMANew

x.normMAOld <- calcNormFactors(x.MAOld, method="TMM")
x.normMAOld$samples
x.normMAOld
...
lcpmMANew <- cpm(x.MANew,log=TRUE)
cpmMANew <- cpm(x.MANew)
colnames(lcpmMANew) <- samplenames.MANew
colnames(cpmMANew) <- samplenames.MANew
View(lcpmMANew)
lcpmMANew <- data.frame(names=row.names(lcpmMANew),lcpmMANew)
cpmMANew <- data.frame(names=row.names(cpmMANew),cpmMANew)

lcpmMAOld <- cpm(x.MAOld,log=TRUE)
cpmMAOld <- cpm(x.MAOld)
colnames(lcpmMAOld) <- samplenamesMAOld
colnames(cpmMAOld) <- samplenamesMAOld
lcpmMAOld <- data.frame(names=row.names(lcpmMAOld),lcpmMAOld)
cpmMAOld <- data.frame(names=row.names(cpmMAOld),cpmMAOld)

rownames(lcpmGC) <- c()
rownames(cpmGC) <- c()
rownames(lcpmMANew) <- c()
rownames(lcpmMAOld) <- c()
...
lcpmGC.test <- merge(genesGC,lcpmGC, by.x="GENEID", by.y="names")
lcpmGC <- lcpmGC.test
lcpmMANew.test <- merge(genesMANew,lcpmMANew, by.x="GENEID", by.y="names")
lcpmMANew <- lcpmMANew.test
lcpmMAOld.test <- merge(genesMAOld,lcpmMAOld, by.x="GENEID", by.y="names")
lcpmMAOld <- lcpmMAOld.test


cpmGC.test <- merge(genesGC,cpmGC, by.x="GENEID", by.y="names")
cpmGC <- cpmGC.test
write.csv(cpmGC,file="cpm_normGC.csv")
cpmMANew.test <- merge(genesMANew,cpmMANew, by.x="GENEID", by.y="names")
cpmMANew <- cpmMANew.test
write.csv(cpmMANew,file="cpm_normMANew.csv")
cpmMAOld.test <- merge(genesMAOld,cpmMAOld, by.x="GENEID", by.y="names")
cpmMAOld <- cpmMAOld.test
write.csv(cpmMAOld,file="cpm_normMAOld.csv")
...
rm(i)
for (i in 3:19) {
  pdf(paste("Mean-Variance Relationship",lines.G[i],".pdf",sep=""))
...
rm(i)
for (i in 3:11) {
  pdf(paste("Mean-Variance Relationship",lines.MANew[i],".pdf",sep=""))
...
for (i in 3:11) {
  pdf(paste("Dist_Variance_MANew",i,".pdf",sep=""))
...
rm(i)
for (i in 3:18) {
  pdf(paste("Mean-Variance Relationship",lines.MAOld[i],".pdf",sep=""))
