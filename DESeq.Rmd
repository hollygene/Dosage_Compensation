...
FDR.res1Def <- fdrtool(res1Def$pvalue,statistic="pvalue",plot=T)
...
#remove the genes filtered out by independent filtering and dispersion outliers 
res1C <- res1[!is.na(res1$pvalue),]
res1C <- res1[!is.na(res1$padj),]
#remove original adjusted p values
res1C <- res1C[,-which(names(res1C)=="padj")]
res1C
summary(is.na(res1C))
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res1C <- fdrtool(res1C$pvalue,statistic="pvalue",plot=T)
#FDR.res1C$param[1,"sd"]
#new estimated null model variance is 0.8267165, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
summary(res1C)
res1C[,"padj"] <- p.adjust(FDR.res1C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res1C$pval,col="deeppink1", main="1 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
table(res1C[,"padj"] < 0.1)
sigGenes1 <- rownames(subset(res1C,padj<0.1))
#genesSig1 <- as.data.frame(subset(genes, GENEID %in% sigGenes1))
summary(res1C)

###########line 2 
#remove the genes filtered out by independent filtering and dispersion outliers
res2C <- res2[!is.na(res2$pvalue),]
res2C <- res2[!is.na(res2$padj),]
#remove original adjusted p values
res2C <- res2C[,-which(names(res2C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res2C <- fdrtool(res2C$pvalue,statistic="pvalue",plot=T)
FDR.res2C <- fdrtool(res2C$stat,statistic="normal",cutoff.method="fndr",plot=T)
FDR.res2C$param[1,"sd"]
#new estimated null model variance is 0.8267165, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res2C[,"padj"] <- p.adjust(FDR.res2C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res2C$pval,col="deeppink1", main="2 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
table(res2C[,"padj"] < 0.1)
sigGenes2 <- rownames(subset(res2C,padj<0.1))
summary(res2C)
#genesSig2 <- as.data.frame(subset(genes, GENEID %in% sigGenes2))

#remove the genes filtered out by independent filtering and dispersion outliers 
res21C <- res21[!is.na(res21$pvalue),]
res21C <- res21[!is.na(res21$padj),]
#remove original adjusted p values
res21C <- res21C[,-which(names(res21C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res21C <- fdrtool(res21C$pvalue,statistic="pvalue",plot=T)
FDR.res21C$param[1,"sd"]
#new estimated null model variance is 0.8267165, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res21C[,"padj"] <- p.adjust(FDR.res21C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res21C$pval,col="deeppink1", main="21 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
table(res21C[,"padj"] < 0.1)
summary(res21C)
sigGenes21 <- rownames(subset(res21C,padj<0.1))
sigGenes21
genesSig21 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes21))
pval.estimate.eta0(res21C$pvalue,method="quantile",diagnostic.plot = TRUE)
##do the same for line 7 that actually has 3 replicates (21 might just need to be discarded)
#remove the genes filtered out by independent filtering and dispersion outliers 
res7C <- res7[!is.na(res7$pvalue),]
res7C <- res7[!is.na(res7$padj),]
#remove original adjusted p values
res7C <- res7C[,-which(names(res7C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res7C <- fdrtool(res7C$stat,statistic="normal",plot=T)
#FDR.res7C$param[1,"sd"]
#new estimated null model variance is 0.6807261, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res7C[,"padj"] <- p.adjust(FDR.res7C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res7C$pval,col="deeppink1", main="7 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 50! 50 more than 0! 
table(res7C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res7C)
summary(res7C)
sigGenes7 <- rownames(subset(res7C,padj<0.1))
genesSig7 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes7))

#remove the genes filtered out by independent filtering and dispersion outliers 
res69C<- res69[!is.na(res69$pvalue),]
res69C <- res69[!is.na(res69$padj),]
#remove original adjusted p values
res69C <- res69C[,-which(names(res69C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res69C <- fdrtool(res69C$stat,statistic="normal",plot=T)
FDR.res69C$param[1,"sd"]
#new estimated null model variance is 0.6807261, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res69C[,"padj"] <- p.adjust(FDR.res69C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res69C$pval,col="deeppink1", main="69 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 50! 50 more than 0! 
table(res69C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res69C)
summary(res69C)
sigGenes69 <- rownames(subset(res69C,padj<0.1))
genesSig69 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes69))
summary(res69C)

#remove the genes filtered out by independent filtering and dispersion outliers 
res3C <- res3[!is.na(res3$pvalue),]
res3C <- res3[!is.na(res3$padj),]
#remove original adjusted p values
res3C <- res3C[,-which(names(res3C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res3C <- fdrtool(res3C$stat,statistic="normal",plot=T)
FDR.res3C$param[1,"sd"]
#new estimated null model variance is 0.6807261, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
FDR.res3C$qval
res3C[,"padj"] <- p.adjust(FDR.res3C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res3C$pval,col="deeppink1", main="3 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 50! 50 more than 0! 
table(res3C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res3C)
sigGenes3 <- rownames(subset(res3C,padj<0.1))
genesSig3 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes3))
summary(res3C)

#remove the genes filtered out by independent filtering and dispersion outliers 
res9C <- res9[!is.na(res9$pvalue),]
res9C <- res9[!is.na(res9$padj),]
#remove original adjusted p values
res9C <- res9C[,-which(names(res9C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res9C <- fdrtool(res9C$stat,statistic="normal",plot=T)
FDR.res9C$param[1,"sd"]
#new estimated null model variance is 0.6807261, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res9C[,"padj"] <- p.adjust(FDR.res9C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res9C$pval,col="deeppink1", main="9 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 50! 50 more than 0! 
table(res9C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res9C)
sigGenes9 <- rownames(subset(res9C,padj<0.1))
genesSig9 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes9))
summary(res9C)

#remove the genes filtered out by independent filtering and dispersion outliers 
res4C <- res4[!is.na(res4$pvalue),]
res4C <- res4[!is.na(res4$padj),]
#remove original adjusted p values
res4C <- res4C[,-which(names(res4C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res4C <- fdrtool(res4C$stat,statistic="normal",plot=T)
FDR.res4C$param[1,"sd"]
#new estimated null model variance is 0.6807261, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res4C[,"padj"] <- p.adjust(FDR.res4C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res4C$pval,col="deeppink1", main="4 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 50! 50 more than 0! 
table(res4C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res4C)
sigGenes4 <- rownames(subset(res4C,padj<0.1))
genesSig4 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes4))
summary(res4C)

#remove the genes filtered out by independent filtering and dispersion outliers 
res5C <- res5[!is.na(res5$pvalue),]
res5C <- res5[!is.na(res5$padj),]
#remove original adjusted p values
res5C <- res5C[,-which(names(res5C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res5C <- fdrtool(res5C$stat,statistic="normal",plot=T)
FDR.res5C$param[1,"sd"]
#new estimated null model variance is 0.6807261, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res5C[,"padj"] <- p.adjust(FDR.res5C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res5C$pval,col="deeppink1", main="5 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 50! 50 more than 0! 
table(res5C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res5C)
sigGenes5 <- rownames(subset(res5C,padj<0.1))
genesSig5 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes5))
summary(res5C)

#remove the genes filtered out by independent filtering and dispersion outliers 
res8C <- res8[!is.na(res8$pvalue),]
res8C <- res8[!is.na(res8$padj),]
#remove original adjusted p values
res8C <- res8C[,-which(names(res8C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res8C <- fdrtool(res8C$stat,statistic="normal",plot=T)
FDR.res8C$param[1,"sd"]
#new estimated null model variance is 0.8807281, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res8C[,"padj"] <- p.adjust(FDR.res8C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res8C$pval,col="deeppink1", main="8 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 80! 80 more than 0! 
table(res8C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res8C)
sigGenes8 <- rownames(subset(res8C,padj<0.1))
genesSig8 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes8))
summary(res8C)

#remove the genes filtered out by independent filtering and dispersion outliers 
res11C <- res11[!is.na(res11$pvalue),]
res11C <- res11[!is.na(res11$padj),]
#remove original adjusted p values
res11C <- res11C[,-which(names(res11C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res11C <- fdrtool(res11C$stat,statistic="normal",plot=T)
FDR.res11C$param[1,"sd"]
#new estimated null model variance is 0.1111072111, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res11C[,"padj"] <- p.adjust(FDR.res11C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res11C$pval,col="deeppink1", main="11 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 110! 110 more than 0! 
table(res11C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res11C)
sigGenes11 <- rownames(subset(res11C,padj<0.1))
genesSig11 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes11))
summary(res11C)

#remove the genes filtered out by independent filtering and dispersion outliers 
res18C <- res18[!is.na(res18$pvalue),]
res18C <- res18[!is.na(res18$padj),]
#remove original adjusted p values
res18C <- res18C[,-which(names(res18C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res18C <- fdrtool(res18C$stat,statistic="normal",plot=T)
FDR.res18C$param[1,"sd"]
#new estimated null model variance is 0.1818072181, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res18C[,"padj"] <- p.adjust(FDR.res18C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res18C$pval,col="deeppink1", main="18 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 180! 180 more than 0! 
table(res18C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res18C)
sigGenes18 <- rownames(subset(res18C,padj<0.1))
genesSig18 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes18))
summary(res18C)

#remove the genes filtered out by independent filtering and dispersion outliers 
res31C <- res31[!is.na(res31$pvalue),]
res31C <- res31[!is.na(res31$padj),]
#remove original adjusted p values
res31C <- res31C[,-which(names(res31C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res31C <- fdrtool(res31C$stat,statistic="normal",plot=T)
FDR.res31C$param[1,"sd"]
#new estimated null model variance is 0.3131072311, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res31C[,"padj"] <- p.adjust(FDR.res31C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res31C$pval,col="deeppink1", main="31 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 310! 310 more than 0! 
table(res31C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res31C)
sigGenes31 <- rownames(subset(res31C,padj<0.1))
genesSig31 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes31))
summary(res31C)

#remove the genes filtered out by independent filtering and dispersion outliers 
res49C <- res49[!is.na(res49$pvalue),]
res49C <- res49[!is.na(res49$padj),]
#remove original adjusted p values
res49C <- res49C[,-which(names(res49C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res49C <- fdrtool(res49C$stat,statistic="normal",plot=T)
FDR.res49C$param[1,"sd"]
#new estimated null model variance is 0.4949072491, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res49C[,"padj"] <- p.adjust(FDR.res49C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res49C$pval,col="deeppink1", main="49 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 490! 490 more than 0! 
table(res49C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res49C)
sigGenes49 <- rownames(subset(res49C,padj<0.1))
genesSig49 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes49))
summary(res49C)

#remove the genes filtered out by independent filtering and dispersion outliers 
res61C <- res61[!is.na(res61$pvalue),]
res61C <- res61[!is.na(res61$padj),]
#remove original adjusted p values
res61C <- res61C[,-which(names(res61C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res61C <- fdrtool(res61C$stat,statistic="normal",plot=T)
FDR.res61C$param[1,"sd"]
#new estimated null model variance is 0.6161072611, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res61C[,"padj"] <- p.adjust(FDR.res61C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res61C$pval,col="deeppink1", main="61 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 610! 610 more than 0! 
table(res61C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res61C)
sigGenes61 <- rownames(subset(res61C,padj<0.1))
genesSig61 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes61))
summary(res61C)

#remove the genes filtered out by independent filtering and dispersion outliers 
res59C <- res59[!is.na(res59$pvalue),]
res59C <- res59[!is.na(res59$padj),]
#remove original adjusted p values
res59C <- res59C[,-which(names(res59C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res59C <- fdrtool(res59C$stat,statistic="normal",plot=T)
FDR.res59C$param[1,"sd"]
#new estimated null model variance is 0.5959072591, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res59C[,"padj"] <- p.adjust(FDR.res59C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res59C$pval,col="deeppink1", main="59 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 590! 590 more than 0! 
table(res59C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res59C)
sigGenes59 <- rownames(subset(res59C,padj<0.1))
genesSig59 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes59))
summary(res59C)

#remove the genes filtered out by independent filtering and dispersion outliers 
res66C <- res66[!is.na(res66$pvalue),]
res66C <- res66[!is.na(res66$padj),]
#remove original adjusted p values
res66C <- res66C[,-which(names(res66C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res66C <- fdrtool(res66C$stat,statistic="normal",plot=T)
FDR.res66C$param[1,"sd"]
#new estimated null model variance is 0.6666072661, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res66C[,"padj"] <- p.adjust(FDR.res66C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res66C$pval,col="deeppink1", main="66 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 660! 660 more than 0! 
table(res66C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res66C)
sigGenes66 <- rownames(subset(res66C,padj<0.1))
genesSig66 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes66))
summary(res66C)


#remove the genes filtered out by independent filtering and dispersion outliers 
res76C <- res76[!is.na(res76$pvalue),]
res76C <- res76[!is.na(res76$padj),]
#remove original adjusted p values
res76C <- res76C[,-which(names(res76C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res76C <- fdrtool(res76C$stat,statistic="normal",plot=T)
FDR.res76C$param[1,"sd"]
#new estimated null model variance is 0.7676072761, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res76C[,"padj"] <- p.adjust(FDR.res76C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res76C$pval,col="deeppink1", main="76 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 760! 760 more than 0! 
table(res76C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res76C)
sigGenes76 <- rownames(subset(res76C,padj<0.1))
genesSig76 <- as.data.frame(subset(genesNoAneu, GENEID %in% sigGenes76))
summary(res76C)

#remove the genes filtered out by independent filtering and dispersion outliers 
res77C <- res77[!is.na(res77$pvalue),]
res77C <- res77[!is.na(res77$padj),]
#remove original adjusted p values
res77C <- res77C[,-which(names(res77C)=="padj")]
#use the z-scores from DESeq2 as input to fdrtool to re-estimate p values
FDR.res77C <- fdrtool(res77C$stat,statistic="normal",plot=T)
FDR.res77C$param[1,"sd"]
#new estimated null model variance is 0.7777072771, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res77C[,"padj"] <- p.adjust(FDR.res77C$pval, method="BH")
#plot the histogram of the "Correct" pvalues 
hist(FDR.res77C$pval,col="deeppink1", main="77 vs Anc, correct null model", xlab="CORRECTED p values")
#test how many of the genes are now differentially expressed with this new null model
###now there are 770! 770 more than 0! 
table(res77C[,"padj"] < 0.1)
#plot the MA plot
plotMA(res77C)
sigGenes77 <- rownames(subset(res77C,padj<0.1))
...
#remove original adjusted p values
res77C <- res77C[,-which(names(res77C)=="padj")]
FDR.res77C <- fdrtool(res77C$pvalue,statistic="pvalue",cutoff.method="locfdr",plot=T)
#FDR.res77C$param[1,"sd"]
#new estimated null model variance is 0.7777072771, lower than 1 (theoretical one)
#add the new BH-adjusted p-values to the results frame 
res77C[,"padj"] <- p.adjust(FDR.res77C$pval, method="BH")
...
```{r exploring results, include=TRUE}
#MA plots 
{pdf(file="MA.1.pdf")
...
dev.off()}
{pdf(file="MA.7.pdf")
plotMA(res7C, ylim=c(-2,2))
dev.off()}
{pdf(file="MA.8.pdf")
plotMA(res8, ylim=c(-2,2))}
{pdf(file="MA.9.pdf")
plotMA(res9, ylim=c(-2,2))
dev.off()}
{pdf(file="MA.11.pdf")
plotMA(res11, ylim=c(-2,2))
dev.off()}
{pdf(file="MA.18.pdf")
plotMA(res18, ylim=c(-2,2))
dev.off()}
{pdf(file="MA.21.pdf")
plotMA(res21C, ylim=c(-2,2))
dev.off()}
{pdf(file="MA.31.pdf")
plotMA(res31C, ylim=c(-2,2))
dev.off()}
{pdf(file="MA.49.pdf")
plotMA(res49, ylim=c(-2,2))
dev.off()}
{pdf(file="MA.59.pdf")
plotMA(res59, ylim=c(-2,2))
dev.off()}
{pdf(file="MA.61.pdf")
plotMA(res61C, ylim=c(-2,2))
dev.off()}
{pdf(file="MA.66.pdf")
plotMA(res66C, ylim=c(-2,2))
dev.off()}
{pdf(file="MA.69.pdf")
plotMA(res69C, ylim=c(-2,2))
dev.off()}
{pdf(file="MA.76.pdf")
plotMA(res76, ylim=c(-2,2))
dev.off()}
{pdf(file="MA.77.pdf")
plotMA(res77, ylim=c(-2,2))
dev.off()}
